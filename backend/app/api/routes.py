...
from app.services.order_flow import (
    mark_order_paid,
    reject_payment as reject_order_payment,
    submit_payment_proof as apply_payment_proof,
    transition_merchant_order,
)
...
def submit_payment_proof(
    ...
    apply_payment_proof(session, order, payment, proof_image_url)
    session.commit()
...
def update_merchant_order(order_id: int, payload: MerchantOrderStatusUpdate, session: Session = Depends(get_session)):
    ...
    if not order:
        raise HTTPException(status_code=404, detail="Order not found")
    transition_merchant_order(session, order, payload.order_status)
    session.commit()
    session.refresh(order)
    return order
...
def reject_merchant_payment(order_id: int, session: Session = Depends(get_session)):
    ...
    if not payment:
        raise HTTPException(status_code=404, detail="Payment not found")
    reject_order_payment(session, order, payment)
    session.commit()
    session.refresh(order)
    session.refresh(payment)
    return {"order": dump(order), "payment": dump(payment)}
